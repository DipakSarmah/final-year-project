...
import { useNavigate } from 'react-router-dom'
import { useQuery } from '@tanstack/react-query'
import { handleFetchAllTeammates, fetchProjectAllocation } from '../api/student'
import { useEffect, useRef, useState } from 'react'
import Modal from '../components/Modal'
import ProjectPreferences from '../components/students/ProjectPreferences'
import { useAppContext } from '../hooks/useContextHooks'
...
function Student() {
  ...
  const userDetailsRef = useRef()
  const { user, updatedIsProjectAllocationForStudent } = useAppContext()
  const navigate = useNavigate()
  const enrolliD = user.enrollment_id
  const sem = user.sem
  const cgpa = user.cgpa
  ...
  // const teamAllocation = useQuery({
  //   queryKey: ['team-allotment'],
  //   queryFn: () => fetchProjectAllocation(enrolliD),
  // })

  useEffect(() => {
    if (fetchAllTeammates.data) {
      const teamId = fetchAllTeammates.data.teamDetails[0].team_id
    ...
    <div className="min-h-screen bg-black text-white p-6">
      ...
      <section>
        ...
        <button
          ...
          disabled={isProjectAllocated} // Disable the button if project is allocated
        >
          Click Here to Set Preferences
        </button>
        <div>
